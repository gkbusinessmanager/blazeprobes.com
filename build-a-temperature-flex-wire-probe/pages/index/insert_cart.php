...
if (isset($_REQUEST['calid'])) {
    ...
    // $links = mysql_connect('localhost', 'totalwe3_wp255', 'wp255@123');
    ...
    if (mysqli_num_rows($oldrecord) && $ttl != "") {
		...
		$product_ttl = str_replace("Custom-Flex-Probe-","",$oldrg['post_title']);
		...
		wp_set_object_terms( $product_id, array( 'flexwire-temperature-probes', 'custom' ), 'product_cat' );
		
		// Set featured image (thumbnail)
		$attachment_id = 218694;
        ...
        /*mysql_query("INSERT INTO ply_posts SET 
							...
							to_ping					= ' ',
							pinged					= ' ',
							guid					= '<?php echo HTTP_HOST; ?>/build-a-temperature-flex-wire-probe/img/flex-home.jpg',
		...
		if($_REQUEST['returntype'] == "both")
		{
			// echo $product_id."{|}Custom-Flex-Probe-".$prt;
			echo $product_id."{|}".$prt;
		}
		else
		{
