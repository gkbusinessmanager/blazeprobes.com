...
if(isset($_REQUEST['calid']))
{
	...
	// $info=$getInfo='';
	// $getInfo = $ob->get_recs("customer","*","part_number='$prtnum' limit 1");      
	if($getInfo):
		foreach($getInfo as $info):
			$sensor_type = $info->sensor_type;
			$connecting_heads = $info->sensor_od;
			$bend_angle = $info->bend_angle;
			$junction_type = $info->junction_type;
			$elements = $info->transition_type;
	...
	//$links = mysql_connect('localhost', 'totalwe3_wp255', 'w60POS5j!@');
	...
	$oldrecord = mysqli_query($links, "select ID,post_title from  ply_posts where post_name = '".$ttl."' limit 1");
	if(mysqli_num_rows($oldrecord))
	{
		$oldrg=  mysqli_fetch_array($oldrecord);
		$product_id=$oldrg['ID']; 
        $product_ttl = $oldrg['post_title'];
		$product_ttl = str_replace("Custom-EGT-Probe-","",$oldrg['post_title']);
	...
	'post_name'      => $lgprt,
	'post_title'     => 'Custom-EGT-Probe-'.$prt,
	'post_status'    => 'publish',
	'post_type'      => 'product',
	'post_author'    => '4',
	'ping_status'    => 'closed',
	...
	comment_status               = 'open',
	ping_status         = 'closed',
	post_name    = 'Custom-EGT-Probe-".$prt."',
	to_ping  = ' ',
	pinged           = ' ',
	...
	$obLink->dblink();
	$links = mysqli_connect('localhost', 'blazepro_conf_usr', 'w60POS5j!@');
	mysqli_select_db($links, 'blazepro_egt_conf') or die('Unable to select database, Please Check Your Connection..');
	$oldrecord = mysqli_query($links, "UPDATE customer SET prod_id=".$product_id." WHERE id= ".$_SESSION['query_id']."");
	mysqli_close($links);
		if($_REQUEST['returntype'] == "both")
		{
			// echo $product_id."{|}Custom-EGT-Probe-".$prt;
			echo $product_id."{|}".$prt;
		}
		else
		{
