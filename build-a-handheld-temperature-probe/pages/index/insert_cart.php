...
if(isset($_REQUEST['calid'])){
	...
	if($getInfo):
        ...
        // $info=$getInfo='';
        $getInfo = $ob->get_recs("customer","*, (select option_name from `option_tbl` op where op.id = customer.sensor_type ) as sensor_type, (select option_name from `option_tbl` op where op.id = customer.sensor_od ) as sensor_od, (select option_name from `option_tbl` op where op.id = customer.junction_type ) as junction_type, (select option_name from `option_tbl` op where op.id = customer.bend_angle ) as bend_angle, (select option_name from `option_tbl` op where op.id = customer.hardware ) as hardware, (select option_name from `option_tbl` op where op.id = customer.extension ) as extension, (select option_name from `option_tbl` op where op.id = customer.flex_armor ) as flex_armor, (select option_name from `option_tbl` op where op.id = customer.terminals ) as terminals, (select option_name from `option_tbl` op where op.id = customer.elements ) as elements, (select option_name from `option_tbl` op where op.id = customer.handle_type ) as handle_type","part_number='$prtnum' limit 1");
    ...
    if (mysqli_num_rows($oldrecord) && $ttl != "") {
			...
			$product_ttl = str_replace("Custom-HH-Probe-","",$oldrg['post_title']);
             mysqli_close($links);
           $obLink->dblink();
...
$postarr = array(
  ...
  'post_name'      => $lgprt,
  'post_title'     => 'Custom-HH-Probe-'.$prt,
  'post_status'    => 'publish',
  'post_type'      => 'product',
  'post_author'    => '4',
  'ping_status'    => 'closed',
...
$wpdb->close();
	...
	/// added by me 28-jan //
		 ...
		 mysqli_query($links, "INSERT INTO ply_posts SET 
								...
								guid = '<?php echo HTTP_HOST; ?>/build-a-rtd-temperature-probe/img/RTD-MAIN.jpg',
								post_content_filtered            = ' ',
	 ...
	 $img_id=mysqli_insert_id($links);	
		...
		if($_REQUEST['returntype'] == "both")
		{
			// echo $product_id."{|}Custom-HH-Probe-".$prt;
			echo $product_id."{|}".$prt;
		}
		else
		{
