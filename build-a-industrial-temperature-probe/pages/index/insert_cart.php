...
if(isset($_REQUEST['calid'])){
	...
	if($getInfo):
		...
		foreach($getInfo as $info):
			...
			$connecting_heads = $info->connecting_heads;
	...
	$pcontant= "<ul>"
        . "<li>Sensor Types: $sensor_type</li>"
        . "<li>Connecting Heads: $connecting_heads</li>"
	...
	mysqli_select_db($links, 'blaze_11252024') or die('Unable to select database, Please Check Your Connection..');
		...
		if(mysqli_num_rows($oldrecord) && $ttl != "") {
			...
			$product_id=$oldrg['ID']; 
			$product_ttl = str_replace("Custom-Industrial-Probe-","",$oldrg['post_title']);
...
$postarr = array(
  ...
  'post_name'      => $lgprt,
  'post_title'     => 'Custom-Industrial-Probe-'.$prt,
  'post_status'    => 'publish',
  'post_type'      => 'product',
  'post_author'    => '4',
  'ping_status'    => 'closed',
...
if( empty( get_post_meta( $product_id, '_sku', true ) ) ) {
	...
	wp_set_object_terms( $product_id, array( 'industrial-probes', 'custom' ), 'product_cat' );
	
	// Set featured image (thumbnail)
	$attachment_id = 218698;
...
$wpdb->close();
	...
	/// added by me 28-jan //
		 ...
		 mysqli_query($links, "INSERT INTO ply_posts SET 
							...
							comment_status               = 'open',
							ping_status         = 'closed',
							post_name    = 'Custom-Industrial-Probe-".$prt."',
							to_ping  = ' ',
							pinged           = ' ',
	 ...
	 $img_id=mysqli_insert_id($links);	
         ...
         mysqli_query($links, $que) or die(mysqli_error($links));
           ...
           $obLink->dblink();
			...
			if($_REQUEST['returntype'] == "both")
			{
				// echo $product_id."{|}Custom-Industrial-Probe-".$prt;
				echo $product_id."{|}".$prt;
			}
			else
			{
